...
fils <- dir("results/finalRes/", pattern="Simu95.+random.+05");fils
...
m.names <- c("MCA", "BOIN", "CCD", "CRM", "Keyboard", "mTPI", "UMPBI")
...
ggplot(data = data, mapping = aes(x = g, y = v, fill = m)) + geom_bar(stat = 'identity', position = 'dodge') + ylim(c(0, 70)) + 
    theme(legend.position = c(0.90, 0.7) , plot.title = element_text(hjust=0.5)) +  xlab("") + ylab("Percentage (%)") + 
    guides(fill=guide_legend(title='Methods')) 
    #ggtitle("Average probability difference around the target = 0.05")
    
ggsave("plots/random_05.jpg", width=10, height = 4.5, units="in")
